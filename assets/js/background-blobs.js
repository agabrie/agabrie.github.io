...
$(document).ready(async () => {
  ...
  $("body")
    .get(0)
    .style.setProperty("--primary-theme-color", `var(--${colorRange.color})`);
  ...
  blobs.forEach(({ $blob_element, x, y, size }) => {
    promises.push(
      new Promise((resolve, reject) => {
        let rotation = getRandomInteger(0, 360);
        let movementconfig = {
          transform: `scale(${getRandomInteger(10, 400)}%) translate(${
            getRandomInteger(0, 50) - 10
          }vw, ${getRandomInteger(0, 50) - 10}vh) rotate(${rotation}deg)`,
